...
struct single_query
{
    net::awaitable< void > start(net::any_io_executor exec, std::string &conn_str)
    {
...

        {
            ...
            CHECK(*large_varchar_column ==
                           "this is a much larger vchar, SPACE SPACE SPACE SPACE SPACE SPACE "
                           "SPACE SPACE SPACE SPACE SPACE SPACE SPACE SPACE SPACE ");
            CHECK(trunc(*float_column * 1000000.) == trunc(3.141592 * 1000000.));
            CHECK(trunc(*double_column * 1000000000000000.) ==
                           trunc(3.141592653589793 * 1000000000000000.));
...
TEST_CASE("async simple query")
{
    ...
    SECTION("1 connection - 1 query") {
        aodbc::net::co_spawn(
            ...
            [](auto ptr){
              exception_handler(ptr,"async test");
            });
